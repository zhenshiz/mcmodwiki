...
<template>
  <div class="size-full flex justify-start items-center flex-col dark:text-white">
    <div class="mt-10 text-[30px] font-bold">{{ translatable('main.title.1') }}</div>
    ...
    <div
      ...
      <div
        ...
        <Button @click="router.push(`/wiki/${translatable(item.lang)}`)" roundedSize="5" isToggleColor class="text-sm mt-3">
          {{ translatable('main.button.1') }}
        </Button>
      </div>
    </div>
  </div>
