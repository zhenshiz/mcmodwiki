...
<ul>
  ...
  <li>
    <p><code>/chatbox mvelTest &lt;表达式：String&gt; &lt;在服务端：Boolean&gt;</code>
      测试一段mvel代码执行情况，最后一个参数填true表示在服务端执行，false表示在客户端执行。执行代码的返回值会发送给玩家，如果执行出错，日志里能看到错误信息（为了避免污染日志，其他用到mvel功能的地方全都不会输出报错）。
    </p>
  </li>
  <li>
    <p><code>/chatbox group</code> 对话组设置，作用仅有成员会自动同步组长的对话进度</p>
    <ul>
      <li>
        <p><code>add &lt;组长：一个玩家&gt; &lt;成员：多个玩家&gt;</code> 新建小组或者往已存在的小组追加成员
        </p>
      </li>
      <li>
        <p><code>remove &lt;一个玩家&gt;</code> 如果该玩家是组长，直接移除小组，否则从小组中移除该成员</p>
      </li>
      <li>
        <p><code>setLeader &lt;一个玩家&gt;</code> 直接把该玩家设置成所在小组的组长</p>
      </li>
      <li>
        <p><code>clear</code> 移除所有小组信息（因为考虑到这个功能的小组不会存在太长时间，直接一键清除会比较方便）</p>
      </li>
    </ul>
  </li>
  <li>
    <p>
      ...
      播放视频，默认不能控制视频进度，不循环，始终允许Esc退出，播放完之后会自动退出</p>
    <div type="important" data-type="admonition" data-admo-type="important">
      ...
      <div data-type="admonition-content">
        <p>本功能需要安装<a href="https://modrinth.com/mod/watermedia" target="_blank"
            rel="noopener noreferrer nofollow">WATERMeDIA</a>模组才能生效！<s>开发环境使用的版本是2.1.25，</s><strong><s>请勿使用3.x版本的watermedia！</s></strong>
        </p>
        <p>
          模组在1.1.5版本之后，<strong>同时兼容</strong>watermedia的2.1.25和3.0.0.23版本，你可以按需选一个版本安装。<strong>MC26.1及以上版本只做了3.0.0.23版本的兼容！</strong>
        </p>
      </div>
    </div>
    <ul>
      <li>
        <p>
          <strong>视频地址</strong>：必填，<strong>注意使用双引号包围</strong>。可以直接填网络视频的url（前提是watermedia支持，如<code>"https://www.bilibili.com/video/BV1vs411W7iK"</code>。特别地，b站视频可以直接填<code>BV1vs411W7iK</code>），也可以填本地文件的路径。可以是绝对路径（例如<code>"D:/games/MC/.minecraft/versions/1.21.1/resourcepacks/video.mp4"</code>），也可以是从当前游戏版本根目录开始的相对路径（如上面那个路径可以写为<code>"resourcepacks/video.mp4"</code>），建议使用相对路径。<strong>特别注意如果watermedia版本为2.x，完整路径不能包含中文字符！</strong>
        </p>
      </li>
    </ul>
    <ul>
      <li>
        <p>
          json文本参数的写法跟对话文本的json基本一样，特殊之处仅在多了一个<code>title</code>参数，<strong>在指令这里需要自己严格注意格式</strong>，具体有哪些参数请看后文介绍。此处只给一个例子，你可以自己测试效果：<code>/chatbox playVideo "BV1vs411W7iK" true true {"title":"天ノ弱", "x":"25", "y":"25", "width":"50", "height":"50"}</code>
        </p>
      </li>
    </ul>
  </li>
...
<ul>
  ...
  <li>
    <p><code>width</code>: 组件的宽度，<strong>单位与下一个参数相关</strong>。</p>
  </li>
  <li>
    <p><code>widthReference</code>:
      宽度参考，默认值为<code>screen_width</code>。可选值包括：<code>screen_width</code>（相对窗口宽度的百分比），<code>screen_height</code>（相对窗口高度的百分比），<code>pixel</code>（绝对像素值，不建议用）。
    </p>
  </li>
  <li>
    <p><code>height</code>: 组件的高度，<strong>单位与下一个参数相关</strong>。</p>
  </li>
  <li>
    <p><code>heightReference</code>:
      高度参考，默认值为<code>screen_height</code>。可选值同<code>widthReference</code>参数。</p>
  </li>
  <li>
...
<p>这里的立绘是预设的状态，将在写对话文本的时候调用立绘的标识符来使用。立绘有4种类型，不同类型的立绘参数作用略有不同。话不多说，上案例：</p><pre language="json" isclosed="false"><code class="language-json">{
  "portrait": {
    "pic": {
      "type": "texture",
...
<p>
  首先<code>pic head item player</code>都是唯一的标识符，你也可以理解成是这个立绘的名称，这些标识符只能出现一次，但是取名可以随便取，用于在对话框文本中调用。
</p>
<ul>
  <li>
    <p>
      <code>type</code>（可选，默认值为<code>texture</code>）：立绘的类型，可选值有<code>texture, player_head, item, entity</code>。<code>texture</code>表示使用资源包的图片，<code>player_head</code>表示使用玩家皮肤的头像，<code>item</code>表示使用物品纹理，<code>entity</code>表示生物实体渲染，比较特殊，见额外的介绍。
    </p>
  </li>
</ul>
<div type="important" data-type="admonition" data-admo-type="important">
...
<ul>
  ...
  <li>
    <p>
      <code>isLock</code>（可选）：立绘是否锁定，如果锁定就会显示锁定后的材质（需要指定<code>lockTexture</code>），建议搭配<code>renderEvents</code>使用<span data-type="hidden-text">（不然不能解锁的话是何意味呢）</span>。比如示例里面的物品立绘会在玩家身上有钻石时显示为钻石（解锁了），否则显示为屏障（锁定）。
    </p>
  </li>
  <li>
    <p>
      <code>lockTexture</code>（可选）：立绘<code>isLock</code>为<code>true</code>时显示的材质，写法同<code>texture</code>。
    </p>
  </li>
  <li>
...
<h4 id="实体渲染" data-toc-id="实体渲染">生物实体渲染</h4>
<p>虽然实体渲染是立绘的类型之一，但它的很多参数与立绘效果都不同，因此在此另作介绍。设计成立绘的子类型是为了便于在json文件中编写。</p>
<ul>
  <li>
    <p>
      <code>texture</code>（必填）：要渲染的生物，<strong>必须是生物</strong>，末影水晶、射出的箭等非生物实体无法渲染。目前支持填<code>@s</code>表示玩家自己；若打开的对话指定了足够数量的对话目标，则可以填<code>target1</code>表示第一个目标实体（1可省略），填<code>target2</code>表示第二个目标实体，以此类推。
    </p>
  </li>
  <li>
    <p><code>alignX, alignY, x, y, scale</code>：效果与其他组件的相同。</p>
  </li>
  <li>
    <p>
      <code>width, height</code>（建议填）：实体渲染的区域，单位为像素。类似于物品的渲染，宽和高本来是无效的，只是这里恰好可以用来限制渲染区域大小。
    </p>
  </li>
  <li>
    <p><code>yOffset</code>（可选）：垂直偏移量。比如你只想渲染玩家的上半身，可以把这个值设置成合适的正数（向下移）。</p>
  </li>
  <li>
    <p>
      <code>stareAt</code>（可选）：实体看向窗口的位置选项。填<code>mouse</code>为看向鼠标（仅screen模式可用）；填<code>point</code>为看向固定位置；否则默认看向正前方。
    </p>
  </li>
  <li>
    <p>
      <code>stareAtX, stareAtY</code>（可选）：<code>stareAt</code>为<code>point</code>时有效，单位为百分比。
    </p>
  </li>
  <li>
    <p><strong>其他参数统统无效！</strong></p>
  </li>
</ul>
...
<p>在介绍对话文件的架构前，先介绍一下我对对话部分的架构设计，这将有利于你理解为什么要这么写这个文件。我的对话架构总结一下就是这么一张图：</p>
<div code="graph TD
    A[对话1] --&gt; B[分支1]
    A --&gt; C[分支2]
    B --&gt; D[对话2]
...
<ul>
  ...
  <li>
    <p><code>bgm</code>：用于控制背景音乐播放。</p>
    <ul>
      <li>
        <p>若值有效，只要玩家处于对话框界面，该音效就会循环播放；</p>
      </li>
      <li>
        <p>连续的对话若该字段值相同，背景音乐不会重新开始；</p>
      </li>
      <li>
        <p>
          填写<strong>空字符串</strong>会<strong>停止</strong>背景音乐（不想为了bgm再加个类似于stopBgm的字段）；
        </p>
      </li>
      <li>
        <p>如果不写该字段（即为null的情况），不会有任何操作（意思是你可以只在需要播放和停止背景音乐时才写这个字段）。</p>
      </li>
    </ul>
  </li>
  <li>
    <p>
      <code>stayTick</code>：强制玩家在该对话停留时长，若未达到指定时长，玩家无法跳转下一句对话（默认值为0，填负数则不论过多久玩家都无法跳转，不影响选项的跳转）。
    </p>
  </li>
  <li>
...
<ul>
  ...
  <li>
    <p>
      <code>unlockCommand</code>（可选）：用于判断选项是否会有特殊状态的指令或者mvel。指令必须以<code>execute</code>开头才会有效。若该指令测试通过，则选项是正常可选状态，否则选项会被上锁（<code>isLock</code>为<code>true</code>）或隐藏（<code>isLock</code>为<code>false</code>或未定义）；mvel的写法详情参考<a
...
<div type="important" data-type="admonition" data-admo-type="important">
  ...
  <div data-type="admonition-content">
    <p>选项通过自己的<code>hidden</code>字段隐藏和因为<code>unlockCommand</code>
      测试不通过而隐藏的机制是不一样的。前者只是客户端渲染层面上的隐藏，可以通过组件渲染事件恢复；后者是服务端隐藏，比较彻底，无法通过除了<code>set_normal</code>类型以外的组件渲染事件恢复。
    </p>
  </div>
</div>
<ul>
...
<ul>
  <li>
    <p>
      <code>path</code>（必填）：可以直接填网络视频的url（前提是watermedia支持，如<code>"https://www.bilibili.com/video/BV1vs411W7iK"</code>。特别地，b站视频可以直接填<code>"BV1vs411W7iK"</code>），也可以填本地文件的路径。可以是绝对路径（例如<code>"D:/games/MC/.minecraft/versions/1.21.1/resourcepacks/video.mp4"</code>），也可以是从当前游戏版本根目录开始的相对路径（如上面那个路径可以写为<code>"resourcepacks/video.mp4"</code>），建议使用相对路径。<strong>特别注意如果watermedia版本为2.x，完整路径不能包含中文字符！</strong>
    </p>
  </li>
  <li>
    <p>
...
<ul>
  ...
  <li>
    <p>
      <code>removeOnEnd</code>（可选，默认<code>true</code>）：视频播放结束后是否立即移除，若为<code>false</code>，视频播放结束前会暂停。
    </p>
  </li>
  <li>
    <p>
      <code>removeOnNext</code>（可选，默认<code>true</code>）：进入下一句对话后是否移除视频，若为<code>false</code>，<strong>视频只会在播放新的视频时被替换</strong>！如果你需要视频在某句对话被移除，你需要手动在该对话的前一句对话使用<code>renderEvents</code>将此字段设置为<code>true</code>。写法如下：
    </p>
  </li>
