...
const searchList = computed(() => {
  ...
  for (const [k, item] of Object.entries(chatBoxEditorStore.themeSetting.portrait)) {
    if (
      (searchInfo.value.name ? item.value.includes(searchInfo.value.name) : true) &&
...
const setPortrait = () => {
  ...
  if (isEditMode.value) {
    ...
    if (
      portraitKey.value !== formData.value.key &&
      formData.value.key in chatBoxEditorStore.themeSetting.portrait
    ) {
...
// 拖拽相关状态
const isDragOverJson = ref(false)
...
const handleJsonDrop = (event) => {
  ...
  if (file && file.name.endsWith('.json')) {
    ...
    message.warning(translatable(lang.value, 'message.warn.json.file'))
  }
}
...
<template>
  <div class="flex flex-row items-center justify-between">
      ...
      <template #content>
        <div class="flex flex-col justify-center w-full dark:text-white">
          ...
          <div class="w-full grid grid-cols-1 sm:grid-cols-2 p-2 m-2">
            ...
            >
              <div>
                {{ translatable(lang, 'chat.box.component.global.portrait.get.component.key')
                }}{{ getDefaultIfInvalid(item.key, '') }}
              </div>
              <div class="flex flex-col center" v-if="item.type?.toUpperCase() === 'TEXTURE'">
                <div>
                  {{ translatable(lang, 'chat.box.component.global.portrait.get.texture') }}
                </div>
                <div>
                  {{ translatable(lang, 'chat.box.component.global.portrait.get.texture.value') }}
                </div>
                <div>{{ item.value }}</div>
                <div>
                ...
                <div
                  v-show="![undefined, 'NONE', 'CUSTOM'].includes(item.animation?.toUpperCase())"
                >
                ...
                <div>
                  {{ translatable(lang, 'chat.box.component.global.portrait.get.component.width')
              ...
              <div class="flex flex-col center" v-if="item.type?.toUpperCase() === 'PLAYER_HEAD'">
                <div>
                  {{ translatable(lang, 'chat.box.component.global.portrait.get.player.head') }}
                </div>
                <div>
              ...
              <div class="flex flex-col center" v-if="item.type?.toUpperCase() === 'ITEM'">
                ...
                <div>
                  {{ translatable(lang, 'chat.box.component.global.portrait.get.item.value')
                  }}{{ item.value }}
                </div>
              </div>
              <div>
                {{ translatable(lang, 'chat.box.component.global.portrait.get.component.x')
                }}{{ getDefaultIfInvalid(item.x, 0) }}
              </div>
              <div>
                {{ translatable(lang, 'chat.box.component.global.portrait.get.component.y')
                }}{{ getDefaultIfInvalid(item.y, 0) }}
              </div>
              <div>
                {{ translatable(lang, 'chat.box.component.global.portrait.get.component.alignX')
                }}{{ getDefaultIfInvalid(item.alignX, 'LEFT') }}
              </div>
              <div>
                {{ translatable(lang, 'chat.box.component.global.portrait.get.component.alignY')
                }}{{ getDefaultIfInvalid(item.alignY, 'TOP') }}
              </div>
              <div>
                {{ translatable(lang, 'chat.box.component.global.portrait.get.component.opacity')
                }}{{ getDefaultIfInvalid(item.opacity, 100) }}
              </div>
              <div>
                {{ translatable(lang, 'chat.box.component.global.portrait.get.component.scale')
              ...
              <div>
                {{
                  translatable(
                    lang,
                    'chat.box.component.global.portrait.get.component.renderOrder'
                  )
              ...
              <Icon
                class="mt-3 cursor-pointer hover:text-red-600"
                width="30"
                @click="deletePortrait(item)"
          ...
          <div class="w-full flex flex-row flex-wrap items-center mb-3">
            ...
            >
              {{
                translatable(
                  lang,
                  'chat.box.component.global.portrait.translatable.generation.json'
                )
              }}
            </Button>
            <div
              class="relative inline-block"
              @dragover.prevent="isDragOverJson = true"
              @dragleave.prevent="isDragOverJson = false"
              @drop.prevent="handleJsonDrop($event)"
            >
              <Button
                :rounded-size="10"
                is-toggle-color
                :class="{ 'border-2 border-blue-500': isDragOverJson }"
          ...
          <table>
            ...
            <tr>
              ...
              <th v-for="(col, index) in chatBoxEditorStore.translatableKeyColumns" class="p-2">
                <div
                  class="text-2xl mb-2"
                  v-if="index < 2 && (col === 'zh_cn' || col === 'en_us')"
                >
                  {{ col }}
                </div>
                <div v-else class="center">
                  <Input
                    class="max-w-[70px] mb-1"
                ...
                <div class="center">
                  <div
                    class="text-sm hover:text-text-blue cursor-pointer"
                    @click="chatBoxEditorStore.removeTranslatableKeyCol(index)"
                  >
                    {{
                      translatable(
                        lang,
                        'chat.box.component.global.portrait.translatable.delete.col'
                      )
                    }}
                  </div>
                </div>
            ...
            <tbody>
            <tr
              v-for="(row, index) in chatBoxEditorStore.translatableKeyRows"
              :key="index"
              class="m-2"
            >
              <td class="p-2">
                <Input
                  v-model="chatBoxEditorStore.translatableKeyRows[index].key"
                  default-model="search"
                />
              </td>
              <td v-for="(cell, colIndex) in row.value" :key="colIndex" class="p-2">
                <Input
                  v-model="chatBoxEditorStore.translatableKeyRows[index].value[colIndex]"
                  default-model="search"
                />
              </td>
              <td>
                <div class="center">
                  ...
                  >
                    {{
                      translatable(
                        lang,
                        'chat.box.component.global.portrait.translatable.delete.row'
                      )
                    }}
                  </div>
                </div>
    ...
    >
      <template #content>
        <Form>
          <FormItem :label="translatable(lang, 'chat.box.component.global.portrait.set.key')">
          ...
          <div class="mb-5" v-if="formData.type?.toUpperCase() === 'TEXTURE'">
            <FormItem
            ...
            >
              <Select
                class="max-w-[150px]"
                is-no-case-sensitive
                v-model:value="formData.animation"
            ...
            <FormItem
              ...
              :label="translatable(lang, 'chat.box.component.global.portrait.set.texture.duration')"
            >
            ...
            >
              <Select
                ...
                class="max-w-[210px]"
                is-no-case-sensitive
                v-model:value="formData.easing"
                :options="easing"
                @update:value="(arg) => (formData.easing = arg.value)"
              />
            </FormItem>
            <FormItem
              :label="translatable(lang, 'chat.box.component.global.portrait.set.component.width')"
            >
              <Input v-model="formData.width" default-model="search" />
            </FormItem>
            <FormItem
              :label="translatable(lang, 'chat.box.component.global.portrait.set.component.height')"
            >
              <Input v-model="formData.height" default-model="search" />
            </FormItem>
          </div>
          <div class="mb-5" v-if="formData.type?.toUpperCase() === 'PLAYER_HEAD'">
            <FormItem
              :label="
          ...
          <div class="mb-5" v-if="formData.type?.toUpperCase() === 'ITEM'">
            <FormItem
          ...
          <FormItem
            :label="translatable(lang, 'chat.box.component.global.portrait.set.component.opacity')"
          >
    ...
    <!--自定义动画-->
    <Modal
      :title="translatable(lang, 'chat.box.component.global.portrait.custom.animation')"
      :show="isShowCustomAnimation"
      :default-close-event="['onClose', 'onNegativeClick']"
      @defaultClose="
        (arg) => {
    ...
    >
      <template #content>
        <div class="flex flex-col gap-2 center w-full dark:text-white">
          ...
          <div
            class="gap-3 border rounded p-3 min-w-[550px]"
          ...
          >
            ...
            <div class="flex flex-row gap-3 items-center">
              ...
              <Icon
                width="20"
                @click="customAnimation.splice(index, 1)"
                class="cursor-pointer hover:text-red-600"
                icon="material-symbols:delete-outline"
              />
            </div>
            <Form v-show="item.visible">
              <FormItem
