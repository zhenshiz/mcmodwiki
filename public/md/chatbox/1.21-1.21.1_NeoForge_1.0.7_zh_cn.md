...
* tooltip（可选）：鼠标悬浮时弹出的文本
  ...
  本功能需要自行去下载[WATERMeDIA](https://modrinth.com/mod/watermedia)

* path（可选）：播放视频的路径，可以是绝对路径，也可以是从当前游戏根目录(.minecraft)开始的相对路径,推荐使用相对路径，并且路径不能包含中文
...
* opacity（可选，默认100）：透明度，范围为`0-100`

* renderOrder（可选，默认-1）：渲染顺序的权重
