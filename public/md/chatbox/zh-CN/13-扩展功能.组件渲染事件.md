...
<ul>
  <li>
    ...
    <ul>
      ...
      <li>
        <p>
          <code>tick</code>：只要玩家打开了对话框，就会每tick触发，不论该组件是否锁定或隐藏。建议设置合理的<code>condition</code>。在客户端执行的mvel中，可用<code>chatboxTick</code>表示当前对话已开始的tick，该值会在进入新的一句对话时重置为0。
        </p>
      </li>
      <li>
        <p>
          <code>check</code>：无论组件是否锁定或隐藏，只要进入对话，必定触发一次，一般用于配合<code>set_normal</code>（见下文）效果恢复组件。
        </p>
      </li>
    </ul>
  </li>
...
<ul>
  ...
  <li>
    <p>
      <code>condition</code>：事件触发条件。默认留空即无条件；默认客户端执行mvel，若以<code>execute</code>开头就是服务端执行命令，若以<code>server:</code>开头就是在服务端执行mvel。条件测试通过后，才会执行事件的效果。<span data-type="hidden-text">不过反正这里都能执行mvel代码了，你其实可以就在这一步把该干的事干了</span>
    </p>
  </li>
</ul>
<p></p>
<ul>
  ...
  <li>
    ...
    <ul>
      ...
      <li>
        <p>
          <code>mvel</code>：执行mvel代码，默认在客户端执行，若参数以<code>server:</code>开头，则会在服务端执行。
        </p>
      </li>
      <li>
        <p>
      ...
      <li>
        <p><code>show</code>：显示特定组件。参数写法同<code>hide</code>。</p>
      </li>
      <li>
        <p>
          <code>replace</code>：替换自身为特定组件。就是先隐藏自己，再显示别的组件的简单写法，参数写法同<code>hide</code>。
        </p>
      </li>
      <li>
        <p><code>lock</code>：锁定特定组件。参数写法同<code>hide</code>。</p>
      </li>
      <li>
        <p><code>unlock</code>：解锁特定组件。参数写法同<code>hide</code>。</p>
      </li>
      <li>
        <p><code>set_normal</code>：取消组件的锁定和隐藏状态。参数写法同<code>hide</code>。</p>
      </li>
      <li>
        <p>
...
<div type="important" data-type="admonition" data-admo-type="important">
  ...
  <div data-type="admonition-content">
    <p>
      需要注意的是，若组件因<code>hide</code>事件或组件的<code>hidden</code>字段为<code>true</code>被隐藏了，那个组件将不会被渲染、不可互动，自然也不可触发除了<code>tick</code>和<code>check</code>以外时机的渲染事件。<br>立绘必须在一句对话的<code>portrait</code>数组中被提及（或者因之前的对话保留）才可显示或隐藏，<strong>无法通过事件添加一个新的立绘到当前的对话中!</strong>
    </p>
  </div>
</div>
<p></p>
...
<div type="info" data-type="admonition" data-admo-type="info">
  ...
  <div data-type="admonition-content">
    <ol>
      ...
      <li>
        <p>
          选项的<code>unlockCommand</code>字段其实就是给选项增加了一个触发时机为<code>check</code>，条件为字段值，<code>set_normal</code>类型，参数为<code>@s</code>的渲染事件。
        </p>
      </li>
    </ol>
  </div>
